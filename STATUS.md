...
PR3 — Standalone collector merged as pull request #8; GitHub issue #7 is closed. PR4 — Visitor hashing & salts merged as pull request #10; GitHub issue #9 is closed. PR5 — Ingest pipeline is ready to merge; GitHub issue: #11.
...
PR5 adds `analytics:ingest`, closed-buffer ordering, token-owned claim leases, bounded resumable line checkpoints, malformed-line tolerance, and a transient sanitized `ingest_events` stage committed before buffer deletion. `./scripts/tm.sh test` is green with exactly-once, active/reclaimed lease, budget, resume, collector-contract, and IPv4/IPv6 staging-privacy coverage; Composer validation and the CI Compose configuration are also green.
