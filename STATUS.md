...
PR9 — Dashboard API & SPA is ready for review; GitHub issue: #19. It adds bounded reporting queries over hourly and exact daily aggregates, explicitly labelled prior-period comparison precision, dimension breakdowns, minimum event-goal conversion counts/rates with daily rollup/retention, and a five-minute aggregate powering the honestly labelled current last-30-minutes report. The authenticated Vue/TypeScript SPA includes all §19 navigation screens with complete `en`/`pt-BR` text, accessible table alternatives, site-timezone rendering, documented ingestion lag, and visible ingest-freshness, shed-event, and cardinality-cap operational warnings. `./scripts/tm.sh test` passed 67 tests / 413 assertions; `vue-tsc`, the Vite build, targeted Pint, and the Docker-run runtime licence audit also passed. Final review found no P0/P1 issues.
